...
class LiloSpec extends FlatSpec with Matchers {
  ...
  "A Lilo" should "batch calls to services" in {
    ...
    Await.result(enrichedTweets.run) shouldBe Seq(
  ...
  it should "be able to be used in complex nested fetches" in {
    ...
    Await.result(enrichedTimelines.run) shouldBe Seq(
...
class TweetRepository {
  def tweetsFor(ids: Seq[Long]): Future[Map[Long, Tweet]] = Future.value(ids.map(id => id -> Tweet(s"Tweet$id", id * 10)).toMap)
}

class UserRepository {
  def usersFor(ids: Seq[Long]): Future[Map[Long, User]] = Future.value(ids.map(id => id -> User(s"User$id")).toMap)
}

class TimelineRepository {
  def timelinesFor(ids: Seq[Long]): Future[Map[Long, Timeline]] = Future.value(ids.map(id => id -> Timeline(List(id * 10, id * 20))).toMap)
}

class LikeRepository {
  def likesFor(ids: Seq[Long]): Future[Map[Long, Like]] = Future.value(ids.map(id => id -> Like(
    List(id * 100, id * 200), List(id * 1000, id * 2000))).toMap)
}

class TrackRepository {
  def tracksFor(ids: Seq[Long]): Future[Map[Long, Track]] = Future.value(ids.map(id => id -> Track(s"Track$id")).toMap)
}