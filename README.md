...
1. ```get``` [creates](https://github.com/getclump/clump/blob/master/src/main/scala/clump/ClumpSource.scala#L12) a ```ClumpFetch``` instances that is the AST element [representing the fetch](https://github.com/getclump/clump/blob/master/src/main/scala/clump/Clump.scala#L81). It doesn't trigger the actual fetch, only [uses](https://github.com/getclump/clump/blob/master/src/main/scala/clump/Clump.scala#L84) the ```ClumpFetcher``` instance to [produce](https://github.com/getclump/clump/blob/master/src/main/scala/clump/ClumpFetcher.scala#L11) a ```Future``` that will be [executed by](https://github.com/getclump/clump/blob/master/src/main/scala/clump/ClumpFetcher.scala#L17) the ```ClumpContext``` when the execution is triggered. The ```ClumpFetcher``` also [removes failed fetches](https://github.com/getclump/clump/blob/master/src/main/scala/clump/ClumpFetcher.scala#L13) in order to enable retrying already failed fetches.

2. ```flatMap``` [creates](https://github.com/getclump/clump/blob/master/src/main/scala/clump/Clump.scala#L16) a ```ClumpFlatMap``` instance [representing the operation](https://github.com/getclump/clump/blob/master/src/main/scala/clump/Clump.scala#L108). It just [composes a new future](https://github.com/getclump/clump/blob/master/src/main/scala/clump/Clump.scala#L115) that is based on the result of the initial Clump and the result of the nested Clump.

3. ```map``` [creates](https://github.com/getclump/clump/blob/master/src/main/scala/clump/Clump.scala#L14) a ```ClumpMap``` instance [representing the map operation](https://github.com/getclump/clump/blob/master/src/main/scala/clump/Clump.scala#L13). It [composes a new future](https://github.com/getclump/clump/blob/master/src/main/scala/clump/Clump.scala#L116) by applying the specified transformation.

Note that __any__ Clump composition creates a ```ClumpContext``` [implicitly](https://github.com/getclump/clump/blob/master/src/main/scala/clump/Clump.scala#L12) if it doesn't exist yet. The ```ClumpContext``` is maintained using a ```Local``` [value](https://github.com/getclump/clump/blob/master/src/main/scala/clump/ClumpContext.scala#L47), that is a [mechanism](https://github.com/twitter/util/blob/master/util-corehttps://github.com/getclump/clump/blob/master/src/main/scala/com/twitter/util/Local.scala#L91) similar to a ```ThreadLocal``` but for asynchronous compositions.  
...
1. ```result``` that has the ```Future``` [result](https://github.com/getclump/clump/blob/master/src/main/scala/clump/Clump.scala#L45) for the operation
2. ```upstream``` that [returns](https://github.com/getclump/clump/blob/master/src/main/scala/clump/Clump.scala#L43) the upstream Clump instances that were used as the basis for the composition
3. ```downstream``` that [returns](https://github.com/getclump/clump/blob/master/src/main/scala/clump/Clump.scala#L44) the downstream Clump instances created as a result of the operation
...
These methods are used by the ```ClumpContext``` to apply the execution model. It has a [collection](https://github.com/getclump/clump/blob/master/src/main/scala/clump/ClumpContext.scala#L12) with all ```ClumpFetcher``` instances in the composition.
...
The ```list``` method is just a [shortcut](https://github.com/getclump/clump/blob/master/src/main/scala/clump/Clump.scala#L26) to ease getting the value of Clump instances that have a ```List```. The actual execution is triggered by the ```get``` [method](https://github.com/getclump/clump/blob/master/src/main/scala/clump/Clump.scala#L36). It flushes the context and returns the Clump's result.

The [context flush](https://github.com/getclump/clump/blob/master/src/main/scala/clump/ClumpContext.scala#L22) is a recursive function that performs simple steps:

* If there aren't Clump instances to be fetched, [stop the recursion](https://github.com/getclump/clump/blob/master/src/main/scala/clump/ClumpContext.scala#L24);
* If there are Clump instances to be fetched
  * [Flush](https://github.com/getclump/clump/blob/master/src/main/scala/clump/ClumpContext.scala#L33) all the upstream instances of the current clumps;
  * [Flush](https://github.com/getclump/clump/blob/master/src/main/scala/clump/ClumpContext.scala#L28) performs all the fetches among the current Clump instances being executed.
  * [Flush](https://github.com/getclump/clump/blob/master/src/main/scala/clump/ClumpContext.scala#L36) all the downstream instances, since the pre-requisite to run the downstream is fulfilled (upstream already flushed). Note that the difference from the ```upstream``` flush is due the fact that ```downstream``` returns a future, but the semantic is the same.
